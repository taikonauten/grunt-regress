...
function initDirs(options) {
  ...
  var referenceDir = path.resolve(dest, 'reference');
  var actualDir = path.resolve(dest, 'actual');
  var diffDir = path.resolve(dest, 'diff');
...
function loop(scenarios, options, func) {
  ...
  return initDirs(options).then(function(folders){
    ...
    return Promise.all(scenarios.map(function (scenario) {
      ...
      var scenario = _.extend({}, scenario, options);
...
function renderReport(options, results) {
  ...
  return Promise.all([index, styles, scripts]).then(function(arr){
    ...
    data.styles = arr[1];
    data.script = arr[2];
    data.results = _.compact(results);
    data.flatResult = _.compact(_.flatten(results));
